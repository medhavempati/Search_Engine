...
class Inverted_Index():
    ...
    def new_word(self, word, doc, field):
        self.index[word] = [[doc, 0, 0, 0, 0, 0, 0]]
        self.index[word][0][field] += 1

    def new_doc(self, word, doc, field):
        self.index[word].append([doc, 0, 0, 0, 0, 0, 0])
        self.index[word][-1][field] += 1

    def add_entry(self, word, doc, field=1):
        try:
            i = [w[0] for w in self.index[word]].index(doc)
            self.index[word][i][field] += 1
        except:
            self.new_doc(word, doc, field)
